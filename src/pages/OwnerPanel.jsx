...
const TABS = [
  ...
  { id: 'about',      icon: '🏠', label: 'About Section'   },
...
function TabDashboard({ setTab }) {
  ...
  const handleReload = async () => {
    ...
    setReloading(false)
    setReloadMsg(ok ? '✓ Data reloaded from database!' : '⚠ Could not reach database.')
  ...
  return (
    <div>
      ...
      <div style={{marginTop:'40px', padding:'24px', background:'rgba(200,16,46,0.06)', border:'1px solid rgba(200,16,46,0.2)', borderRadius:'4px'}}>
        <p className="op-section-title" style={{fontSize:'14px',marginBottom:'8px'}}>🔄 Data Recovery</p>
        <p className="op-hint" style={{marginBottom:'16px'}}>If anything looks wrong or reset, click this to reload all your saved data fresh from the database.</p>
        {reloadMsg && <p style={{fontSize:'13px', color: reloadMsg.startsWith('✓') ? '#22c55e' : '#f59e0b', marginBottom:'12px'}}>{reloadMsg}</p>}
        <button className="btn-outline" onClick={handleReload} disabled={reloading} style={{fontSize:'12px',padding:'10px 20px'}}>
...
function TabAbout() {
  ...
  const [toast, setToast] = useState('')
  const set = (k,v) => setForm(p=>({...p,[k]:v}))
  const save = async () => { const ok = await updateSection('about', form); setToast(ok?'Saved ✓':'Error'); setTimeout(()=>setToast(''),2500) }

  return (
    <div>
      {toast && <div className="op-toast">{toast}</div>}
      <p className="op-hint">This controls the "About Us" section on the home page — the image, heading, sub-heading, and body text.</p>

      <ImageField
        label="ABOUT SECTION IMAGE"
        description="Photo shown beside the About Us text on the home page. Use a gym interior shot or action photo."
        value={form.image||''}
        onChange={v=>set('image',v)}
      />

      <OpField label="Heading" hint="e.g. More Than A Gym.">
        <input value={form.heading||''} onChange={e=>set('heading',e.target.value)} placeholder="More Than A Gym." />
      </OpField>

      <OpField label="Sub-heading" hint="One short line below the heading">
        <input value={form.subheading||''} onChange={e=>set('subheading',e.target.value)} placeholder="A Place Where Discipline Becomes Identity." />
      </OpField>

      <OpField label="Body Text" hint="2–3 sentences describing the gym">
        <textarea rows={4} value={form.body||''} onChange={e=>set('body',e.target.value)}
          placeholder="Knockout Gym has been Zirakpur's premier training facility since 2019…" />
      </OpField>
  ...
  const upload = async (e) => {
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        alert('Upload failed. Try again or paste an image URL instead.')
      }
    } catch {
        ...
        {!isVideo && (
          <div className="img-upload-option">
            ...
            <div>
              ...
              <p className="op-field-hint" style={{marginTop:'6px'}}>Photo uploads directly to Cloudinary CDN — no size limits.</p>
            </div>
          </div>
        )}
      </div>
